...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SubmitBlock(blockHex, callback) {
        ...
        _this.daemon.cmd(rpcCommand,
            rpcArgs,
            function (results) {
                console.log('---rpcCommand result00:');
            console.log(result);
            console.log(results);
    ...
    // const hexString = "00010203"; // Replace with your hex string
    // const byteBuffer = hexToBytes(hexString);




    // const crypto = require('crypto');
    function blockhexToBlockhash(blockHex) {
        
        
        const x22i = require('node-x22i');
        ...
        // Sample Bitcoin block hex string (replace with your actual block hex string)
        // const blockHex = "";
        
        function decodeBlockHex(blockHex) {
...
// Parse the block using bitcoinjs-lib
const parsedBlock = bitcoin.Block.fromBuffer(blockBuffer);

// Access individual block fields
const version = parsedBlock.version;
const previousBlockHash = parsedBlock.prevHash;
const merkleRoot = parsedBlock.merkleRoot;
const timestamp = parsedBlock.timestamp;
const bits = parsedBlock.bits;
const nonce = parsedBlock.nonce;

console.log('Version:', version);
console.log('Previous Block Hash:', previousBlockHash.toString('hex'));
console.log('Merkle Root:', merkleRoot.toString('hex'));
console.log('Timestamp:', timestamp);
console.log('Bits:', bits);
console.log('Nonce:', nonce);
        
            const reconstructedBlockHeader = version + previousBlockHash + merkleRoot + timestamp + bits + nonce;
        
            // Decode transactions (this is a simplified example)
            const transactionsHex = blockHex.slice(168);
        
            return reconstructedBlockHeader;
        }
        
        const reconstructedBlockHeader = decodeBlockHex(blockHex);
        
        // Calculate the X22i hash
        const blockBuffer = hexToBytes(reconstructedBlockHeader);
        const x22iHash = x22i.x22i(blockBuffer);
        
        console.log('X22i Hash:', x22iHash);
    }        
    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            console.log("isValidBlock ------------ blockHex");
            ...
            if (!isValidBlock)
                ...
                if (jobManagerLastSubmitBlockHex === blockHex) {
                    ...
                    console.log('------------------');
                    const test = blockhexToBlockhash(blockHex);
                    ...
                    SubmitBlock(blockHex, function () {
                        CheckBlockAccepted(shareData.blockHash, function (isAccepted, tx) {
                            ...
                            GetBlockTemplate(function (error, result, foundNewBlock) {
                                if (foundNewBlock) {
                                    emitLog('---Block notification via RPC after block submission---'); // 008
                                }
                            });
                        });
                    });
    ...
    function DetectCoinData(finishedCallback) {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            ...
            options.protocolVersion = '008';

            options.initStats = {
                connections: rpcResults.getnetworkinfo.connections,
                difficulty: rpcResults.getdifficulty.difficulty * algos[options.coin.algorithm].multiplier,
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            GetBlockTemplate(function (error, rpcData, processedBlock) {
                console.log('---rpcData:', rpcData); // 008
    ...
    function SetupBlockPolling() {
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(function (error, result, foundNewBlock) {
                if (foundNewBlock)
                    emitLog('---Block notification via RPC polling ---- 2 ----');
                    emitLog('---foundNewBlock:');
                    emitLog(foundNewBlock); // false/true
            });
        }, pollingInterval);
    }
    ...
    function GetBlockTemplate(callback) {
        // console.log('Submitting request:', [{"capabilities": ["coinbasetxn", "workid", "coinbase/append"], "rules": "[segwit"]}]);
// 008
        _this.daemon.cmd('getblocktemplate',
            ...
            function (result) {
                ...
                // console.log(result);
                if (result.error) {
                    // console.log('-- Error getblocktemplate --:', result.error); // 008
                    ...
                    // console.log('-- Response getblocktemplate -- :', result.response); // 008
    ...
    function CheckBlockAccepted(blockHash, callback) {
        //setTimeout(function(){
            console.log('blockHash to check:');
        ...
        _this.daemon.cmd('getblock',
            ...
            function (results) {
                ...
                console.log('---do we have any results?');
